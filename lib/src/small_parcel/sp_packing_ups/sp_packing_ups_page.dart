...
class SPPackingUpsPage extends StatefulWidget {
  const SPPackingUpsPage({
    super.key,
  });

  @override
  State<SPPackingUpsPage> createState() => _CreateState();
}

class _CreateState extends State<SPPackingUpsPage> {
  ...
  Future<(List<SPPackingUpsModel>, bool)> _getData() async {
    List<SPPackingUpsModel> orderData = [];
    try {
      for (var element in SPPackingUpsColumn.data) {
        final data = SPPackingUpsModel.fromJson(element);
        orderData.add(data);
      }
  ...
  Widget build(BuildContext context) {
    return CustomScaffold(
      ...
      title: 'Small Parcel / SP Packing Ups',
      body: Column(
        ...
        children: [
          ...
          SizedBox(
            height: 200.h,
            child: OperanceDataTable<SPPackingUpsModel>(
              showRowsPerPageOptions: false,
              showFooter: false,
              columns: SPPackingUpsColumn.leftColumns,
          ...
          SizedBox(
            ...
            child: OperanceDataTable<SPPackingUpsModel>(
              showRowsPerPageOptions: false,
              showFooter: false,
              columns: SPPackingUpsColumn.rightColumns,
              emptyStateBuilder: (BuildContext context) => EmptyWidget(),
              onFetch: (limit, sort, {bool isInitial = true}) async {
                return _getData();
              },
